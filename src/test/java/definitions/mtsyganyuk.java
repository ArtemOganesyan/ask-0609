...
public class mtsyganyuk {
    ...
    @Then("^MT wait for element with xpath \"([^\"]*)\" to be present$")
    ...
    @Then("^MT wait for element with xpath \"([^\"]*)\" to not be present$")
    ...
    @When("^MT mouse over element with xpath \"([^\"]*)\"$")
    ...
    @Then("MT put {int} characters into the field with xpath{string}")
    public void iPutCharactersIntoTheFieldWithXpath(int number0fCahrs, String xpath) {
        String val = "b".repeat(number0fCahrs);
        getDriver().findElement(By.xpath(xpath)).sendKeys(val);
    }

    @And("MT put {int} chars befor @ sign and {int} chars after and {int} chars after dot for element at xpath{string}")
    public void iPutCharsBeforSignAndCharsAfterAndCharsAfterDotForElementAtXpath(int firstPart, int secondPart, int domainPart, String xpath) {
    ...
    @And("MTT clear element with xpath {string}")
    public void iBackSpaceElementWithXpath(String xpath) throws InterruptedException {
        for (int i = 0; i < 100; i++) {
